...
music() {
    while true; do
        NCMP=$(ncmpcpp --current-song | grep -E "^(\[playing\]|\(\-\:\-\-\))" | awk '{print $1}')
        NUM_NCMP=$(ncmpcpp --current-song {%t} | wc -c )
        S_NCMP=$(ncmpcpp --current-song {%t} | head -1 | head -c 30)

        pause='Pause'
        str=""

        if [ "$NCMP" = "[playing]" ] || [ "$NCMP" = "(-:--)" ]; then        
